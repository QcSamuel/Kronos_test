...
// 32 Mbit Dram
//
// Memory map - Sega Saturn Technical Bulletin #47 (Preliminary),
// "SEGASaturn Extended RAM Cartridge Manual Ver. 1.02", Table 1:
//
//                        8 Mbit                       32 Mbit
//   DRAM0    0x2400000-0x247FFFF (512KB)    0x2400000-0x25FFFFF (2MB)
//   DRAM1    0x2600000-0x267FFFF (512KB)    0x2600000-0x27FFFFF (2MB)
//
// The 8Mbit cart leaves 0x2480000-0x25FFFFF and 0x2680000-0x27FFFFF
// unmapped because its two 512KB banks sit inside 2MB windows. The 32Mbit
// cart has NO such gaps: its two 2MB banks fill both windows exactly, so
// the whole 0x400000-0x7FFFFF range is backed by real RAM.
//
// Do NOT "align" the 32Mbit layout on the 8Mbit one: punching holes at
// 0x500000-0x5FFFFF / 0x700000-0x7FFFFF makes 2MB of the cart unreadable
// and unwritable, which breaks every game that actually uses the 4MB cart
// (X-Men vs SF, MSH vs SF, Vampire Savior, Metal Slug...) and CART_DEV,
// which shares these handlers.
//
// Note: Die Hard Trilogy freezing right after the BIOS with the 32Mbit
// cart is not an emulation bug - the game is genuinely incompatible with
// the 4MB cart on real hardware (it only recognizes ID 5Ah, exactly the
// case Technical Bulletin #47 section 5 warns about). The fix belongs in
// cart auto-selection (IP.BIN "W" peripheral code / game DB), not here.
//////////////////////////////////////////////////////////////////////////////
...
static u8 FASTCALL DRAM32MBITCs0ReadByte(SH2_struct *context, UNUSED u8* memory, u32 addr)
{
   ...
   switch (addr >> 20)
   {
      case 0x04: // DRAM0 (0x400000-0x5FFFFF)
      case 0x05:
      case 0x06: // DRAM1 (0x600000-0x7FFFFF)
      case 0x07:
         return T1ReadByte(CartridgeArea->dram, addr & 0x3FFFFF);
      default:   // The rest doesn't matter
         break;
