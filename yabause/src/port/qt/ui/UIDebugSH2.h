...
/* Breakpoint callback, shared by the debugger window and the main window
   so that a raised breakpoint is reported even when the debugger was never
   opened. */
...
private:
   ...
   /* Snapshot of the previous register refresh, used purely so
      updateRegList() can flag the values a Step Into/Over/Out (or a
      breakpoint) just changed. Not valid until havePrevRegs is true. */
   sh2regs_struct prevRegs;
   bool havePrevRegs = false;
